...
def create_assessment_prompt(skill_type):
    """Creates the prompt for skill assessment."""
    
    if skill_type == "تمرير":
        prompt = f"""
        مهمتك هي تقييم مهارة التمرير في كرة القدم المعروضة في الفيديو.
        استخدم المعايير التالية لتقييم الأداء:

        **معايير تقييم التمرير:**
        - **ضعيف**: تمريرة خاطئة تماماً أو ضعيفة جداً أو بدون دقة، لا تصل للهدف
        - **متوسط**: تمريرة مقبولة تصل للهدف ولكن بقوة أو دقة متوسطة
        - **جيد**: تمريرة دقيقة ومتقنة بقوة مناسبة، تضع المستلم في وضع جيد

        شاهد الفيديو بعناية وقيّم مهارة التمرير.

        هام جداً: قم بالرد بكلمة واحدة فقط من الخيارات التالية: "ضعيف" أو "متوسط" أو "جيد"
        لا تكتب أي شيء آخر غير هذه الكلمة.
        """
        
    elif skill_type == "استقبال":
        prompt = f"""
        مهمتك هي تقييم مهارة استقبال الكرة في كرة القدم المعروضة في الفيديو.
        استخدم المعايير التالية لتقييم الأداء:

        **معايير تقييم الاستقبال:**
        - **ضعيف**: فشل في السيطرة على الكرة تماماً، الكرة تبتعد كثيراً أو تتطلب جهداً كبيراً للسيطرة
        - **متوسط**: استقبال مقبول، الكرة تحت السيطرة بعد لمستين أو بحركة إضافية
        - **جيد**: استقبال ممتاز، لمسة أولى نظيفة تهيئ الكرة للخطوة التالية بسهولة

        شاهد الفيديو بعناية وقيّم مهارة الاستقبال.
        ...
        prompt = f"""
        مهمتك هي تقييم مهارتي التمرير والاستقبال في كرة القدم المعروضتين في الفيديو.
        استخدم المعايير التالية لتقييم الأداء:

        **معايير تقييم التمرير:**
        - **ضعيف**: تمريرة خاطئة تماماً أو ضعيفة جداً أو بدون دقة
        - **متوسط**: تمريرة مقبولة تصل للهدف ولكن بقوة أو دقة متوسطة
        - **جيد**: تمريرة دقيقة ومتقنة بقوة مناسبة

        **معايير تقييم الاستقبال:**
        - **ضعيف**: فشل في السيطرة على الكرة تماماً
        - **متوسط**: استقبال مقبول، الكرة تحت السيطرة بعد لمستين
        - **جيد**: استقبال ممتاز، لمسة أولى نظيفة

        شاهد الفيديو بعناية وقيّم كلا المهارتين.

        هام جداً: قم بالرد بالتنسيق التالي فقط:
        التمرير: [ضعيف أو متوسط أو جيد]
        الاستقبال: [ضعيف أو متوسط أو جيد]
        ...
        if skill_type == "كلاهما":
            ...
            for line in lines:
                ...
                if 'التمرير:' in line:
                    ...
                    results['التمرير'] = GRADE_MAP.get(grade.lower(), grade)
                elif 'الاستقبال:' in line:
                    grade = line.split(':')[1].strip()
                    results['الاستقبال'] = GRADE_MAP.get(grade.lower(), grade)
            
            if not results:
                # Fallback parsing if format is different
                for grade in ['ضعيف', 'متوسط', 'جيد']:
                    if grade in raw_text:
                        results['التمرير'] = grade
                        results['الاستقبال'] = grade
                        break
                        
            return results if results else {'التمرير': NOT_CLEAR_AR, 'الاستقبال': NOT_CLEAR_AR}
        else:
            # Parse single skill
            grade = None
            for ar_grade in ['ضعيف', 'متوسط', 'جيد']:
                if ar_grade in raw_text:
                    grade = ar_grade
                    break
            
            if not grade:
                # Try English mappings
                for en_grade, ar_grade in GRADE_MAP.items():
                    if en_grade.lower() in raw_text.lower():
                        grade = ar_grade
                        break
                        
            return grade if grade else NOT_CLEAR_AR
...
def display_assessment_result(skill, result):
    """Display the assessment result with styling."""
    if isinstance(result, dict):
        # Both skills
        st.markdown("### 📊 نتائج التقييم")
        for skill_name, grade in result.items():
            css_class = grade if grade in ['ضعيف', 'متوسط', 'جيد'] else 'متوسط'
            st.markdown(f"""
        ...
        # Single skill
        css_class = result if result in ['ضعيف', 'متوسط', 'جيد'] else 'متوسط'
                ...
                try:
                    ...
                    if gemini_file:
                        ...
                        if result:
                            ...
                            if (isinstance(result, dict) and any(grade == 'جيد' for grade in result.values())) or result == 'جيد':
                                st.balloons()
                        else:
